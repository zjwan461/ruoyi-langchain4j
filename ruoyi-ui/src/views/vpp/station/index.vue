<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="${comment}" prop="crtTime">
      ...
      <el-form-item label="${comment}" prop="upTime">
        <el-date-picker clearable
    ...
    <el-table v-loading="loading" :data="stationList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="${comment}" align="center" prop="crtTime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.crtTime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="${comment}" align="center" prop="upTime" width="180">
        <template slot-scope="scope">
...
<script>
import { listStation, getStation, delStation, addStation, updateStation } from "@/api/vpp/station";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除电站编号为"' + ids + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('vpp/station/export', {
        ...this.queryParams
